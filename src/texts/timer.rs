    ...
    fn with_text(s: &str) -> Result<Time, String> {
        ...
        if !current_num.is_empty() {
            ...
            let unit_secs = if let Some(last) = last_unit {
                next_smaller_unit(last).unwrap_or(SECOND).as_secs()
            } else {
                1 // 默认秒
            };
...
mod tests {
    ...
    fn test_parse_combined() {
        assert_eq!(
            Time::with_text("1h30m")
                .unwrap_or(Time::new())
                .get_duration(),
            Duration::from_secs(5400)
        );
        assert_eq!(
            Time::with_text("1d12h")
                .unwrap_or(Time::new())
                .get_duration(),
        ...
        assert_eq!(
            Time::with_text("2w3d")
                .unwrap_or(Time::new())
                .get_duration(),
            Duration::from_secs(1468800)
        );
        assert_eq!(
            Time::with_text("1h30m10s")
                .unwrap_or(Time::new())
                .get_duration(),
            Duration::from_secs(5410)
        );
        assert_eq!(
            Time::with_text("1d2h3m4s")
                .unwrap_or(Time::new())
                .get_duration(),
    ...
    fn test_parse_without_unit() {
        ...
        assert_eq!(
            Time::with_text("1h100")
                .unwrap_or(Time::new())
                .get_duration(),
            Duration::from_secs(9600)
        );
    ...
    fn test_parse_invalid_unit() {
        ...
        assert!(Time::with_text("10y").is_err());
        assert!(Time::with_text("1M").is_err());
    }

    #[test]
    fn test_parse_invalid_format() {
        assert!(Time::with_text("s10").is_err());
        assert!(Time::with_text("hs").is_err());
    }
    ...
    fn test_duration_methods() {
        ...
        let d = Time::with_text("1h30m").unwrap();
