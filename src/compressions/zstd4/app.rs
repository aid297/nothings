...
/// Zstd 压缩工具
///
/// 提供基于 Zstandard (zstd) 格式的压缩/解压功能，支持 1-21 级压缩级别。
/// 压缩级别 1 最快，21 最高压缩率，默认级别 3。
pub struct Zstd {
    level: i32,
}
...
impl Zstd {
    ...
    pub fn compress(data: &[u8]) -> Result<Vec<u8>, Error> {
        Self::compress_with_level(data, 3)
    }
    ...
    /// # 参数
    /// - `data`: 待压缩数据
    /// - `level`: 压缩级别，范围 1-21（1 最快，21 最高压缩率）
    ///
    /// # 错误
    ...
    /// assert_eq!(decompressed, data);
    /// ```
    pub fn compress_with_level(data: &[u8], level: i32) -> Result<Vec<u8>, Error> {
    ...
    ///
    /// # 错误
    /// 如果数据不是有效的 zstd 压缩格式，返回 `std::io::Error`
    ///
    /// # 示例
    /// ```
    /// use aid::compressions::zstd4::app::Zstd;
    ...
    pub fn compress_file(input_path: &str, output_path: &str) -> Result<(), Error> {
        Self::compress_file_with_level(input_path, output_path, 3)
    }
...
impl Compressor for Zstd {
    fn compress(&self, data: &[u8]) -> Result<Vec<u8>, Error> {
        Zstd::compress_with_level(data, self.level)
    }
