...
#[test]
fn test_from_iter() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2), ("c", 3)]);
    assert_eq!(map.to_hashmap().len(), 3);
    assert_eq!(map.get_value_by_key(&"attr").unwrap(), 1);
...
fn test_from_iter_empty() {
    let map: AnyMap<&str, i32> = AnyMap::with_iter(vec![]);
    assert_eq!(map.to_hashmap().len(), 0);
}
...
fn test_from_hashmap() {
    ...
    hm.insert("x", 10);
    hm.insert("y", 20);
    let map = AnyMap::with_hashmap(hm);
...
fn test_set_value_by_index() {
    let mut map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    map.set_value_by_index(0, 100);
    assert_eq!(map.get_value_by_key(&"attr").unwrap(), 100);
}
...
fn test_set_value_by_key_found() {
    let mut map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    let result = map.set_value_by_key("attr", 99);
    assert!(result.is_ok());
    assert_eq!(map.get_value_by_key(&"attr").unwrap(), 99);
}

#[test]
fn test_set_value_by_key_not_found() {
    let mut map = AnyMap::with_iter(vec![("attr", 1)]);
...
fn test_get_value_by_key_found() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    assert_eq!(map.get_value_by_key(&"attr").unwrap(), 1);
}

#[test]
fn test_get_value_by_key_not_found() {
    let map = AnyMap::with_iter(vec![("attr", 1)]);
...
fn test_to_hashmap() {
    ...
    assert_eq!(hm.len(), 2);
    assert_eq!(*hm.get("attr").unwrap(), 1);
...
fn test_copy() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    let copied = map.copy();
    assert_eq!(copied.get_value_by_key(&"attr").unwrap(), 1);
...
fn test_get_indexes_by_keys() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2), ("c", 3)]);
    let indexes = map.get_indexes_by_keys(&vec!["attr", "c"]);
...
fn test_get_indexes_by_keys_partial() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    let indexes = map.get_indexes_by_keys(&vec!["attr", "z"]);
...
fn test_in_keys_true() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2), ("c", 3)]);
    assert!(map.in_keys(&vec![&"attr", &"b"]));
}

#[test]
fn test_in_keys_false() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    assert!(!map.in_keys(&vec![&"attr", &"z"]));
}
...
fn test_not_in_keys_false() {
    let map = AnyMap::with_iter(vec![("attr", 1), ("b", 2)]);
    assert!(!map.not_in_keys(&vec![&"attr", &"z"]));
}
...
fn test_every_short_circuit() {
    ...
    let result = map.every(|k, _v| *k != "attr");
